...
from fleet.models import Company, Driver, Transaction, Truck, Wallet
...
def booking_list(request):
    ...
    bookings = (
        Booking.objects.all()
        .select_related("truck", "user", "load_type")
...
def add_booking(request):
    ...
    if request.method == "POST":
        ...
        price = calculate_admin_booking_price(
            distance_km=distance_km, load_multiplier=load_type.price_multiplier
        )
    ...
    return render(
        request,
        "bookings/add_booking.html",
        {"trucks": trucks, "load_types": load_types},
    )
...
def edit_booking(request, booking_id):
    ...
    if request.user.role == "CUSTOMER":
        ...
        if not (
            ...
            or booking.customer_name == request.user.username
        ...
        if booking.truck:
            messages.error(
                request, "Cannot edit confirmed bookings (truck assigned by company)."
            )
            return redirect("customer_booking_list")
    ...
    trucks = (
        Truck.objects.filter(is_available=True)
        | Truck.objects.filter(id=booking.truck.id)
        if booking.truck
        else Truck.objects.filter(is_available=True)
    )
    load_types = LoadType.objects.all()

    if request.method == "POST":
        booking.customer_name = request.POST.get("customer_name", booking.customer_name)
        booking.pickup_location = request.POST.get(
        ...
        booking.distance_km = float(
            request.POST.get("distance_km", booking.distance_km)
        )
...
def customer_booking(request):
    ...
    return render(
        request,
        "bookings/customer_booking.html",
        {
            "trucks": trucks,
            "load_types": load_types,
...
# =============================================================================
# NEW FEATURES VIEWS (Live GPS + Ratings + Cancel)
# =============================================================================
...
def cancel_booking(request, booking_id):
    """
    NEW: Allow customers to cancel PENDING bookings (before truck assigned).
    """
    if request.user.role != "CUSTOMER":
    ...
    if not (
        ...
        or booking.customer_name == request.user.get_full_name()
        or booking.customer_name == request.user.username
    ):
        messages.error(request, "You can only cancel your own bookings.")
        return redirect("customer_booking_list")

    # Only PENDING bookings before truck assignment
    if booking.status != "PENDING" or booking.truck:
        messages.error(
            request, "Can only cancel pending bookings before truck assignment."
        )
        return redirect("customer_booking_list")

    if request.method == "POST":
        # Release truck if somehow assigned
        if booking.truck:
...
@login_required(login_url="/login/")
def live_tracking_booking(request, booking_id):
    """
    NEW: Live GPS tracking for specific booking.
    Shows map with pickup → truck → dropoff route.
    """
    if request.user.role != "CUSTOMER":
        messages.error(request, "Only customers can track bookings.")
        return redirect("customer_booking_list")
    ...
    if not (
        ...
        messages.error(request, "You can only track your own bookings.")
        return redirect("customer_booking_list")

    truck = booking.truck
    if not truck or not truck.current_latitude or not truck.current_longitude:
        messages.error(request, "No GPS data available for this booking.")
        return redirect("customer_booking_list")
    ...
    context = {
        ...
        "dropoff": {"lat": booking.drop_lat, "lng": booking.drop_lng},
    }
    return render(request, "bookings/tracking_map.html", context)


@login_required(login_url="/login/")
@csrf_exempt
def submit_company_rating(request, company_id):
    """
    NEW: Submit company rating (1-5 stars) after completed booking.
    """
    if request.user.role != "CUSTOMER":
        return JsonResponse(
            {"success": False, "error": "Only customers can rate"}, status=403
        )
    ...
    if request.method == "POST":
        ...
        # Find recent completed booking with this company
        recent_booking = (
            Booking.objects.filter(
                user=request.user, truck__company=company, status="COMPLETED"
            )
            .order_by("-created_at")
            .first()
        )
        ...
        if not recent_booking:
            return JsonResponse(
                {"success": False, "error": "No completed booking found"}
            )

        # Prevent duplicate rating
        if CompanyRating.objects.filter(
            company=company, booking=recent_booking
        ).exists():
            return JsonResponse(
                {"success": False, "error": "Already rated this company"}
            )
        ...
        return JsonResponse(
            {
                "success": True,
                "message": f"Thanks for rating {company.company_name} {rating}/5⭐!",
            }
        )
...
def booking_receipt(request, booking_id):
    ...
    if request.user.role == "CUSTOMER":
        if (
            booking.customer_name != request.user.get_full_name()
            and booking.customer_name != request.user.username
        ):
    ...
    if not weasyprint_available:
        ...
        if request.user.role == "CUSTOMER":
            if (
                ...
                if not request.user.is_staff:
                    messages.error(
                        request, "You are not authorized to download this receipt."
                    )
                    return redirect("customer_dashboard")

        messages.info(
            request, "PDF download is not available. Using browser print instead."
        )
        return render(request, "bookings/booking_receipt.html", {"booking": booking})
...
def driver_dashboard(request):
    ...
    if driver:
        ...
        if view_type == "today":
            driver_bookings = (
                Booking.objects.filter(driver=driver, booking_date=today)
                .select_related("truck", "user", "load_type")
            ...
            driver_bookings = (
                ...
                .select_related("truck", "user", "load_type")
                .order_by("-assigned_at", "-booking_date")[:20]
            )
        ...
        weekly_earnings = (
            all_driver_bookings.filter(
                status="COMPLETED", booking_date__gte=week_ago
...
def driver_jobs(request):
    ...
    driver_bookings = driver_bookings.select_related(
        "truck", "user", "load_type", "driver"
    ...
    if driver.assigned_truck:
        if (
            driver.assigned_truck.current_latitude
            and driver.assigned_truck.current_longitude
        ):
            truck_location = {
...
def driver_update_job_status(request, booking_id, new_status):
    ...
    messages.success(
        request, f"Job #{booking.id} status updated to {new_status.replace('_', ' ')}."
    )
    return redirect("driver_dashboard")
...
def driver_accept_job(request, booking_id):
    ...
    try:
        booking = Booking.objects.get(
            id=booking_id, truck__in=company_trucks, driver=driver
        )
    except Booking.DoesNotExist:
    ...
    messages.success(
        request, f"Job #{booking.id} accepted! You can now start the trip."
    )
    return redirect("driver_dashboard")
...
def driver_reject_job(request, booking_id):
    ...
    if booking.assigned_by_company:
        messages.error(
            request,
            "You cannot reject a job that was assigned by the company. Please contact your manager.",
        )
        return redirect("driver_dashboard")
...
def admin_user_change_role(request, user_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"User '{user.username}' role changed from {old_role} to {user.get_role_display()}.",
        )
        return redirect("admin_user_detail", user_id=user_id)
...
def admin_reset_password(request, user_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Password for '{user.username}' has been reset successfully."
        )
        return redirect("admin_user_detail", user_id=user_id)
...
def admin_truck_add(request):
    ...
    return render(
        request,
        "bookings/admin_truck_add.html",
        {"companies": companies, "truck_types": truck_types},
    )
...
def admin_company_approve(request, company_id):
    ...
    messages.success(
        request, f"Company '{company.company_name}' approved successfully."
    )
...
def admin_load_type_edit(request, load_type_id):
    ...
    return render(
        request, "bookings/admin_load_type_form.html", {"load_type": load_type}
    )
...
def admin_load_type_delete(request, load_type_id):
    ...
    return render(
        request, "bookings/admin_load_type_delete.html", {"load_type": load_type}
    )
...
def admin_subscriptions(request):
    ...
    return render(
        request, "bookings/admin_subscriptions.html", {"subscriptions": subscriptions}
    )
    ...
    return JsonResponse(
        {
            "total_bookings": Booking.objects.count(),
            "total_revenue": float(
                Booking.objects.aggregate(Sum("price"))["price__sum"] or 0
            ),
            "total_users": User.objects.count(),
            "total_trucks": Truck.objects.count(),
            "available_trucks": Truck.objects.filter(is_available=True).count(),
...
def process_payment(request):
    ...
    if request.method == "POST":
        ...
        if not booking_id:
            return JsonResponse(
                {"success": False, "error": "Booking ID is required"}, status=400
            )
        ...
        if payment_method == "CARD":
            ...
            if not card_expiry or len(card_expiry) < 4:
                return JsonResponse(
                    {"success": False, "error": "Invalid expiry date"}, status=400
                )
            if not card_cvv or len(card_cvv) < 3:
                return JsonResponse(
                    {"success": False, "error": "Invalid CVV"}, status=400
                )
...
def faq(request):
    ...
    public_faqs = FAQQuestion.objects.filter(
        status="ANSWERED", is_public=True
    ).order_by("-answered_at")

    # Get user's submitted questions (if logged in)
    user_questions = []
    if request.user.is_authenticated:
        user_questions = FAQQuestion.objects.filter(user=request.user).order_by(
            "-created_at"
        )
...
def faq_submit(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, "Your question has been submitted! We'll get back to you soon."
        )
        return redirect("faq")
...
class EmailAuthenticationForm(AuthenticationForm):
    ...
    username = forms.EmailField(
        ...
        widget=forms.EmailInput(
            attrs={"class": "form-control minimal-select", "placeholder": "Email"}
        ),
    )
    password = forms.CharField(
        label="Password",
        widget=forms.PasswordInput(
            attrs={"class": "form-control minimal-select", "placeholder": "Password"}
        ),
    )
...
def accept_bid(request, bid_id):
    ...
    booking.assigned_by_company = (
        True  # Mark as company assigned - driver cannot reject
    )
    ...
    messages.success(
        request, f"Bid accepted! Truck {bid.truck.truck_number} has been assigned."
    )
    return redirect("customer_dashboard")
        ...
        try:
            ...
            if request.user.role == "COMPANY":
                try:
                    ...
                    return JsonResponse(
                        {"success": False, "error": "Company not found"}, status=403
                    )
            ...
            return JsonResponse(
                {"success": False, "error": "Truck not found"}, status=404
            )
...
def request_payout(request):
    ...
    if request.method == "POST":
        ...
        if amount > wallet.available_balance:
            return JsonResponse(
                {"success": False, "error": "Insufficient balance"}, status=400
            )
        ...
        success = wallet.process_payout(
            amount, f"Payout requested for {company.company_name}"
        )

        if success:
            return JsonResponse(
                {"success": True, "message": "Payout request submitted successfully"}
            )
        else:
            return JsonResponse(
                {"success": False, "error": "Payout failed"}, status=400
            )
...
def process_payment_with_escrow(request):
    ...
    if request.method == "POST":
        ...
        if not booking_id:
            return JsonResponse(
                {"success": False, "error": "Booking ID required"}, status=400
            )
