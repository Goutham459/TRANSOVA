...
def detect_user_currency() -> str:
    ...
    try:
        ...
        logger.warning(
            f"Currency API parse error: {e}. Using default: {default_currency}"
        )
        return default_currency
...
def detect_card_type(card_number: str) -> str:
    ...
    if card_clean.startswith("4"):
        ...
        return "Amex"
    elif card_clean.startswith(("6011", "65")) or card_clean[:3] in [
...
def validate_session_otp(
    request, entered_otp: int, key: str = "otp"
) -> Tuple[bool, str]:
    """
    ...
    if created_timestamp:
        ...
        if elapsed.total_seconds() > (validity_minutes * 60):
            ...
            return (
                False,
                f"OTP has expired. Please request a new one (valid for {validity_minutes} minutes).",
            )
