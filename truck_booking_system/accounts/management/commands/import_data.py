...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if User.objects.exists():
            self.stdout.write(
                self.style.WARNING("Database already has users, skipping data import")
            )
            return
        ...
        possible_paths = [
            os.path.join(
                os.path.dirname(__file__), "..", "..", "..", "database_backup.json"
            ),
            os.path.join(
        ...
        if backup_file:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "database_backup.json not found, skipping data import"
                )
            )
