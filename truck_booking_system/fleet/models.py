...
class Company(models.Model):
    ...
    @property
    def avg_rating(self):
        """Average rating from completed bookings (1-5 stars)."""
        from bookings.models import CompanyRating
    ...
    def rating_count(self):
        ...
        return CompanyRating.objects.filter(company=self).count()

    @property
    def rating_display(self):
        """Formatted rating display (★ 4.2 (12))."""
        avg = self.avg_rating
...
class Wallet(models.Model):
    ...
    company = models.OneToOneField(
        Company, on_delete=models.CASCADE, related_name="wallet"
    )
